...
CREATE TABLE IF NOT EXISTS MARK_ROLE (
                                         ...
                                         ROLE_CODE VARCHAR(100) NOT NULL COMMENT '角色简称',
                                         SORT_NO INT(10) NOT NULL COMMENT '显示顺序',
                                         ROLE_STATUS INT(1) NOT NULL COMMENT '角色状态（1正常 0停用）',
...
-- 3 角色用户关联表
DROP TABLE IF EXISTS MARK_USER_ROLE_RELA;
CREATE TABLE IF NOT EXISTS MARK_USER_ROLE_RELA (
...
CREATE INDEX MARK_USER_ROLE_RELA_USER_ID ON MARK_USER_ROLE_RELA(USER_ID);
CREATE INDEX MARK_USER_ROLE_RELA_ROLE_ID ON MARK_USER_ROLE_RELA(ROLE_ID);
...
CREATE INDEX MARK_ROLE_MENU_RELA_ROLE_ID ON MARK_ROLE_MENU_RELA(ROLE_ID);
CREATE INDEX MARK_ROLE_MENU_RELA_MENU_ID ON MARK_ROLE_MENU_RELA(MENU_ID);
...
-- 1 初始化用户数据
DELETE FROM MARK_USER WHERE ID = 1;
INSERT INTO MARK_USER (`ID`, `USER_ID`, `USER_NAME`, `UER_PASSWORD`, `USER_SALT`, `USER_PHONE`, `USER_EMAIL`, `USER_REGISTER_TIME`, `USER_OPEN_ID`, `USER_QQ`) VALUES (1, 1, 'admin', '$2a$10$aII3cWi9Yjp/tn90qcrZ/ucPzHHkirraaA/wX1lfDgYl0V8t8p22S', 'mark', NULL, NULL, NULL, NULL, NULL);
-- 2 初始化角色数据
DELETE FROM MARK_ROLE WHERE ROLE_ID = 1;
INSERT INTO MARK_ROLE (`ROLE_ID`, `ROLE_NAME`, `ROLE_CODE`, `SORT_NO`, `ROLE_STATUS`, `DEL_FLAG`, `CREATE_TIME`, `REMARK`) VALUES (1, '管理员', 'admin', 1, 1, 0, NULL, '管理员');
-- 3 初始化菜单

-- 4 初始化菜单，角色关联表

-- 5 初始化用户角色关联表
DELETE FROM MARK_USER_ROLE_RELA WHERE ID = 1;
INSERT INTO `MARK_USER_ROLE_RELA` (`ID`, `USER_ID`, `ROLE_ID`) VALUES (1, 1, 1);
